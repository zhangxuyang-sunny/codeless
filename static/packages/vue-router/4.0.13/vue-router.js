define(["vue"], (__WEBPACK_EXTERNAL_MODULE_vue__) => { return /******/ (() => { // webpackBootstrap
/******/ 	"use strict";
...
function onBeforeRouteLeave(leaveGuard) {
  ...
  const activeRecord = vue.inject(matchedRouteKey, // to avoid warning
  {}).value;
...
function useLink(props) {
  ...
  function navigate(e = {}) {
    if (guardEvent(e)) {
      return router[vue.unref(props.replace) ? 'replace' : 'push'](vue.unref(props.to) // avoid uncaught errors are they are logged anyway
      ).catch(noop);
    }
...
const RouterViewImpl = /*#__PURE__*/vue.defineComponent({
  ...
  setup(props, {
    ...
    const viewRef = vue.ref(); // watch at the same time the component instance, the route record we are
