...
import { loadRemoteComponent } from "../core/loadRemoteComponent";
import { useGlobalProperties } from "../core/useGlobalProperties";
import { IMaterialOptionConsumer } from "packages/x-core/src/types/material";
const {
...
export default defineComponent({
  ...
  props: {
    data: {
      type: Object as PropType<IMaterialOptionConsumer | null>,
      default: null
    }
  },
  setup(props) {
    ...
    // 递归节点生成元素树
    const generateComponent = (node: IMaterialOptionConsumer): (() => JSX.Element | null) => {
      ...
      const slots = Object.entries(node.slots) //
        .reduce<Record<string, () => JSX.Element[]>>((slotMapper, [slotName, slot]) => {
          const Slots = (Array.isArray(slot) ? slot : []).map(generateComponent);
